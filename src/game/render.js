/* ============================================================
   ...
   Everything is drawn at the level's own 1:1 resolution and upscaled
   with nearest-neighbour by the caller, so the pixels stay square.

   The HUD is the exception, and deliberately so. It is authored in a
   fixed 448x252 design space and drawn through a scale onto whatever
   the play resolution actually is. That way the raising of the play
   resolution buys the HUD FONT more pixels — which is the whole point
   of raising it — without every hard-coded panel corner in here having
   to be found and multiplied by hand.
   ============================================================ */
window.RENDER = (function () {
  ...
  /* ---------------- HUD ----------------
     Authored against this design space and scaled onto the real frame.
     HUD_W/HUD_H are what every coordinate below is in. */
  const HUD_W = 448, HUD_H = 252;
  const HUD_S = LV.W / HUD_W;              // 1.25 at 560x315
  const FONT = 'bold 8px "Courier New", monospace';

  /* Round a design-space coordinate so it lands on a whole pixel of the
     real frame. A pixel font drawn on a fractional origin is a blurred
     pixel font, and the scale above puts almost everything on quarters
     unless something rounds. */
  const snap = v => Math.round(v * HUD_S) / HUD_S;

  /* Where the warden's text box lives, in design space. The HUD reads
     these to step out of its way, so the box and the things dodging it
     cannot drift. */
  const DIALOG_H = 62;
  const DIALOG_TOP = HUD_H - DIALOG_H - 6;
  ...
  function hud(M, ctx, time) {
    ...
    const W = P.weapon;
    ctx.fillStyle = 'rgba(8,10,11,.74)';
    ctx.fillRect(HUD_W - 118, 4, 114, 26);
    text(ctx, W.def.label, HUD_W - 8, 7, W.def.proto ? (W.def.tint || vis) : vis, 'right');
    if (W.def.proto) {
      ...
      if (bits.length) text(ctx, bits.join(' '), HUD_W - 8, 31, hexA(W.def.tint, 0.9), 'right');
    }
    if (W.reloading > 0) {
      bar(ctx, HUD_W - 114, 16, 106, 4, 1 - W.reloading / W.def.reload, vis);
      text(ctx, 'RELOADING', HUD_W - 8, 23, '#8d9aa2', 'right');
    } else {
      ...
      for (let i = 0; i < cells; i++) {
        ...
        ctx.fillRect(HUD_W - 114 + i * per, 16, Math.max(1, per - 1), 4);
      }
      const spare = P.spare[W.kind];
      text(ctx, W.ammo + ' / ' + (spare === Infinity ? '∞' : spare),
        HUD_W - 8, 23, '#8d9aa2', 'right');
    }
    ...
    text(ctx, 'EXTRACTION', HUD_W / 2, 6, '#8d9aa2', 'center');
    ctx.fillStyle = 'rgba(40,46,50,.9)';
       ...
       than under the vitals, because these are things the gun does. */
    {
      ...
      marks.forEach((m, i) => {
        text(ctx, m[0] + ' x' + m[2].toFixed(1), HUD_W - 8, y0 + i * 7, hexA(m[1], 0.85), 'right');
      });
    }
    ...
    if (!talking) {
      text(ctx, 'HOSTILES ' + (M.totalEnemies - M.kills) + '/' + M.totalEnemies, 4, HUD_H - 18, '#8d9aa2');
      text(ctx, String(P.score).padStart(6, '0'), HUD_W - 4, HUD_H - 18, vis, 'right');
    }

    /* boss bar — only once it knows you are there */
    /* The boss bar steps up above the text box rather than being
       hidden by it: you can still fight through a conversation, and a
       fight you cannot read the health bar for is worse than a
       conversation you have to read around. */
    const O = M.overlord;
    if (O && !O.dead && O.aggroed) {
      const bw = 170, bx2 = Math.round(HUD_W / 2 - bw / 2);
      const by2 = talking ? DIALOG_TOP - 14 : HUD_H - 30;
      ctx.fillStyle = 'rgba(8,10,11,.80)';
      ctx.fillRect(bx2 - 3, by2 - 10, bw + 6, 17);
      text(ctx, 'OVERLORD', HUD_W / 2, by2 - 9, '#c8323a', 'center');
      ...
      text(ctx, 'PHASE ' + O.phase, HUD_W / 2 + bw / 2, by2 - 9, '#8d9aa2', 'right');
    }
    ...
    if (M.autopilot && !talking) {
      ...
      const y = (M.bannerT > 0 && M.banner) ? HUD_H - 54 : HUD_H - 42;
      text(ctx, 'AUTOPILOT', HUD_W / 2, y, hexA('#4ad07a', blink), 'center');
    }
    ...
    if (M.bannerT > 0 && M.banner && !talking) {
      ...
      ctx.globalAlpha = a;
      text(ctx, M.banner, HUD_W / 2, HUD_H - 40, vis, 'center');
      ctx.globalAlpha = 1;
    }
    ...
    if (M.hitFlash > 0) {
      ...
      const g = ctx.createRadialGradient(HUD_W / 2, HUD_H / 2, HUD_H * 0.25, HUD_W / 2, HUD_H / 2, HUD_H * 0.8);
    ...
    if (P.hp < 30 && !P.dead) {
      ...
      const g = ctx.createRadialGradient(HUD_W / 2, HUD_H / 2, HUD_H * 0.2, HUD_W / 2, HUD_H / 2, HUD_H * 0.85);
  ...
  /* ============================================================
     ...
     The prompts and the crosshair point at things in the level, so
     they are in PLAY pixels, not the HUD's design space — a prompt
     over a warden's head that has been through the HUD scale is a
     prompt over somebody else's head.
     ============================================================ */
  function worldHud(M, ctx, time) {
    const P = M.player;
    const vis = P.rig.params.colVisor || '#f0a830';
    const blink = Math.floor(time * 3) % 2 === 0;

    if (M.talkPrompt && !M.dialog.open && blink) {
      const W = M.talkPrompt;
      text(ctx, W.spent ? 'SPEAK' : 'SOMETHING IS HERE',
           W.x - M.scroll, W.y - W.h - 20,
           W.rig.params.colVisor || '#ffb45a', 'center');
    }
    if (M.activatePrompt && blink) {
      const A = M.activatePrompt;
      text(ctx, 'TOUCH TO REVIVE', A.x - M.scroll, A.y - A.h - 18,
    ...
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(cx - 6.5, cy + 0.5); ctx.lineTo(cx - 2.5, cy + 0.5);
    ctx.moveTo(cx + 2.5, cy + 0.5); ctx.lineTo(cx + 6.5, cy + 0.5);
    ctx.moveTo(cx + 0.5, cy - 6.5); ctx.lineTo(cx + 0.5, cy - 2.5);
    ctx.moveTo(cx + 0.5, cy + 2.5); ctx.lineTo(cx + 0.5, cy + 6.5);
    ctx.stroke();
  ...
  function overlay(M, ctx, time) {
    ...
    ctx.fillText(label, HUD_W / 2 + 1, HUD_H / 2 - 7);
    ctx.fillStyle = won ? '#4ad07a' : '#e0552a';
    ctx.globalAlpha = a;
    ctx.fillText(label, HUD_W / 2, HUD_H / 2 - 8);
    ctx.globalAlpha = 1;
    if (M.endT > 1.1) {
      ...
      if (blink) {
        ...
        ctx.fillText(hint, HUD_W / 2, HUD_H / 2 + 14);
      }
    }
  }
  ...
  function frame(M, ctx, canvas) {
    ...
    worldHud(M, ctx, M.time);
    ctx.restore();

    /* The HUD and the run-end overlays are authored in design space and
       scaled onto the frame; everything above this line is authored in
       real play pixels. */
    ctx.save();
    ctx.scale(HUD_S, HUD_S);
    hud(M, ctx, M.time);
